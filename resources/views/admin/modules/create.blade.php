...
<div class="max-w-2xl mx-auto bg-white p-6 rounded shadow">
  ...
  <form method="POST" action="{{ route('admin.modules.store', $course) }}">
    ...
    <div class="mb-4">
      ...
      <textarea name="content" rows="4" class="w-full border px-3 py-2 rounded">{{ old('content') }}</textarea>
    </div>

    <div class="mb-4">
      <label class="block font-medium">Link Video YouTube (opsional)</label>
      <input name="video_url" type="url" class="w-full border px-3 py-2 rounded" value="{{ old('video_url') }}">
    </div>

    <div class="mb-6">
      <label class="block font-medium">Urutan Modul</label>
      <input name="order" type="number" min="1" class="w-full border px-3 py-2 rounded" value="{{ old('order', 1) }}">
    </div>
